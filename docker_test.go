...
func TestBuildDeployment(t *testing.T) {
	...
	if testBuild.Config.MemorySwap != testDeployment.MemorySwap {
		t.Errorf("expected %s, got %s", testDeployment.MemorySwap, testBuild.Config.MemorySwap)
	}
	if testBuild.Config.CpuShares != testDeployment.CPU {
		t.Errorf("expected %s, got %s", testDeployment.CPU, testBuild.Config.CpuShares)
	}
	if testBuild.Config.Image != testDeployment.Image {
		t.Errorf("expected %s, got %s", testDeployment.Image, testBuild.Config.Image)
	}
	if testBuild.Config.Memory != testDeployment.Memory {
		t.Errorf("expected %s, got %s", testDeployment.Memory, testBuild.Config.Memory)
	}
